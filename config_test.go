package config

import (
	"testing"

	"github.com/unlikezy/go-defaults"
)

func init() {
	//仅用于单元测试: 修改默认的配置路径,以便测试用例可以一键通过
	*ConfPath = "./config.toml"
}

//config.toml
//[section]
//k1=11
//k2="v2"
//k3="1s"
//k4=[1, 2, 3]
//k5=100

var confWithDefaultValue = struct {
	Section struct {
		K1 int      `default:"1"` //默认值都必须用""引起来,即使是数值,这是golang的struct tag的要求
		K2 string   `default:"val2"`
		K3 Duration `default:"800ms"`
		K4 []uint64 //数组不支持设置默认值
		K5 string   `default:"v5"`
	}
}{}

var myConf struct {
	MySection struct {
		K5 int `default:"2"`
	}
}

func TestParse(t *testing.T) {
	defaults.SetDefaults(&confWithDefaultValue)
	t.Logf("Defaults:%+v", confWithDefaultValue)

	Parse(&confWithDefaultValue)

	//不同的模块可以定义不同的结构,解析同一个toml文件,获取自己想要的配置
	Parse(&myConf)
	//output:
	//default config:
	//[Section]
	//K1 = 1
	//K2 = "val2"
	//K3 = 0

	//config file:./config.toml
	//toml config:
	//[Section]
	//K1 = 2
	//K2 = "vvv"
	//K3 = 1000000000
	//K4 = [1, 2, 3]

	//default config:
	//[MySection]
	//K5 = 2

	//config file:./config.toml
	//toml config:
	//[MySection]
	//K5 = 100
}
